...
- **Help popup:** `wireHelpPrompt()` opens `#help-dialog` once per page load after `HELP_PROMPT_DELAY_MS` of visible time, deferring while the Add task dialog is open. The delay and `SUPPORT_HOTLINE` are config constants next to `FORMSUBMIT_ENDPOINT`.
